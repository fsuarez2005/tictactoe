...
fun TicTacToePanel(gameViewModel: TicTacToeViewModel = viewModel(), modifier: Modifier = Modifier) {
    ...
    Column {
        ...
        BoardCell(
            ...
            onClick = {
                Log.d(TicTacToeApp.TAG,"BoardCell clicked")

                //tttUiState.board.makeMove(0,0)
                tttUiState.makeMove(0,0)
                //tttUiState.board.switchPlayer()
                tttUiState.switchPlayer()
                //player = tttUiState.board.currentPlayer
                player = tttUiState.currentPlayer

                cellEnabled = !gameViewModel.isCellOccupied(0,0)
            },
            enabled = cellEnabled
